$(document).ready(function () {
    ...
    //* buttonlar və sillabuslar hissəsi

    function toggleSyllabus(buttonId, syllabusClass, buttonTextId) {
        $(buttonId).on('click', function (e) {
            e.preventDefault();
            let $button = $(this);
            let $buttonText = $(buttonTextId);
            let $syllabus = $(syllabusClass);

            if ($syllabus.is(':visible')) {
                $buttonText.text('Göstər');
                $button.removeClass('toggled');
            } else {
                $buttonText.text('Gizlət');
                $button.addClass('toggled');
            }

            $syllabus.slideToggle(800);

        });
    }

    toggleSyllabus('#toggleFourButton', '.four-sillabus', '#buttonFourText');
